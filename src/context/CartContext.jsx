...
export function CartProvider({ children }){
  ...
  const toggleWishlist = (product) => {
    if (wishlist.find(p => p.id === product.id)) removeFromWishlist(product.id)
  ...
  const totalPrice = useMemo(() => items.reduce((s, i) => s + (Number(i.product.newPrice ?? i.product.price ?? 0) * i.qty), 0), [items])
